function [dudx]=gradient_dudxdvdy_compact_rich_bhavini(u,dx,dir)
...
%inputs are 
% N = velocity in pix/second
% dx = grid spacing (pixels/vector_spacing)
% dir = direction (dy-1 dx-2 )
...
% if there is no input for direction use these defaults
if isempty(dir)
    if size_N(1)~=1     % if multiple entries in y direction use dy
        dir = 1;
    elseif size_N(2)~=1 % if multiple entries in x direction but only 1 in y then use dx
        dir = 2;
    elseif size_N(3)~=1 % if multiple entries in z direction but only 1 in y&x then use dx
        dir = 3;
    else
        dir = 1;
    end
    ...
    for m=1:4
        ...
        while c1<=k
            ...
            for l=2:size(N_current,1)-1
             ...
             if numel(N_current) == 1
                 d(1,1)=1/(k(m)*dx)*(0);
                 d(size(N_current,1),1)=1/(k(m)*dx)*(N_current(size(N_current,1),1)-N_current(size(N_current,1),1));
             else
            ...
            [y]=tridiag(a,b,c, d);   %% Solve tridiagonal Matrix
